...
import type { GmatChartData, GmatChartDataset, GmatChartType } from '@/lib/gmat-types';

const SERIES_COLORS = ['#84a8ff', '#f07e25', '#53d3a6', '#f7c95f', '#d181ff', '#5fd5f7'];

type ChartRendererProps = {
  type: GmatChartType;
  data: GmatChartData | null;
};
  ...
  return (
    <section className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5 md:p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#84a8ff]">Chart</p>
          <p className="mt-2 text-sm text-slate-300">{data.title ?? 'Interactive visual reference'}</p>
        </div>
      </div>
      <div className="mt-5 h-[320px] w-full overflow-hidden rounded-[22px] border border-white/10 bg-[#0d1528] p-4">
